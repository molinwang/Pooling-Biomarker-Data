...
data = data.sim.match(OR=1.5,matchratio=1,beta2=0,type=2,errorType = "normal")$data

#############################################
#2, parameter estimation
#############################################

result =main_func(data=data,                ## the dataset name
                  Y="Y",                      ## disease outcome
                  HL="HL",                    ## local lab measurements
                  HC="HC",                    ## central lab measurements 
                  Wname="W",                  ## covariates in the model for X (input NULL if Wname is empty)
                  Zname=NULL,                 ## covariate in the model for Y (input NULL if Zname is empty; currently only accepts 0 or 1 Z-variables)
                  ID="matchID",               ## ID for the matched set or strata
                  group="group",              ## group or study number
                  calibration="calibration"   ## whether the subject is in the calibration subset? (Yes 1, No 0)
)
