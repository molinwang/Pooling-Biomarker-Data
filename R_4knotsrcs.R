...
sp.basis  <- function(knots, x){
  ...
  f2 <- 0 * (x <= knots[1]) + ((x - knots[1])^3)*( x > knots[1] & x <= knots[3]) +
    ( (x - knots[1])^3 - ((x - knots[3])^3*(knots[4]-knots[1]))/(knots[4]-knots[3]))*(x > knots[3] & x <= knots[4]) +
    (  (x - knots[1])^3 - ((x - knots[3])^3*(knots[4]-knots[1]))/(knots[4]-knots[3]) +
         ((x-knots[4])^3*(knots[3]-knots[1]))/(knots[4]-knots[3]))*(x > knots[4])
  
  f3 <- 0 * (x <= knots[2]) + ((x - knots[2])^3)*( x > knots[2] & x <= knots[3]) +
    ( (x - knots[2])^3 - ((x - knots[3])^3*(knots[4]-knots[2]))/(knots[4]-knots[3]))*(x > knots[3] & x <= knots[4]) +
    ((x - knots[2])^3 - ((x - knots[3])^3*(knots[4]-knots[2]))/(knots[4]-knots[3]) +
       ((x-knots[4])^3*(knots[3]-knots[2]))/(knots[4]-knots[3]))*(x > knots[4])
  
  spline.basis <- data.frame(f1 = f1, f2 = f2, f3 = f3)
  return(spline.basis)
}
...
deriv.spline <- function(knots,x){
  ...
  f2.prime <- 0*(x<=knots[1]) + (3*(x-knots[1])^2)*(x>knots[1] & x<=knots[3]) +
    (3*(x-knots[1])^2 - (3*(x-knots[3])^2*(knots[4]-knots[1]))/(knots[4]-knots[3]))*(x>knots[3] & x<=knots[4])+
    (3*(x-knots[1])^2 - (3*(x-knots[3])^2*(knots[4]-knots[1]))/(knots[4]-knots[3])+
       (3*(x-knots[4])^2*(knots[3]-knots[1]))/(knots[4]-knots[3]))*(x>knots[4])
  
  f3.prime <- 0*(x<=knots[2]) + (3*(x-knots[2])^2)*(x>knots[2] & x<=knots[3]) +
    (3*(x-knots[2])^2 - (3*(x-knots[3])^2*(knots[4]-knots[2]))/(knots[4]-knots[3]))*(x>knots[3] & x<=knots[4])+
    (3*(x-knots[2])^2 - (3*(x-knots[3])^2*(knots[4]-knots[2]))/(knots[4]-knots[3])+
       (3*(x-knots[4])^2*(knots[3]-knots[2]))/(knots[4]-knots[3]))*(x>knots[4])
...
int_df = function(data, X, S, H, W, Y, strata, nstud, nref, knots, covar=NA){
  ...
  output_int = matrix(NA, ncol=9, nrow=1) #### to be generalized
  colnames(output_int) = c("point estimate of beta_fx1","point estimate of beta_fx2","point estimate of beta_fx3",
                           "Estimated RR_fx1", "Estimated RR_fx2", "Estimated RR_fx3", 
                           "Estimated variance of beta_fx1","Estimated variance of beta_fx2","Estimated variance of beta_fx3")
...
fc_df = function(data, X, S, H, W, Y, strata, nstud, nref, knots, covar=NA){
  ...
  #### 1. Create matrix to store output
  output_fc = matrix(NA, ncol=9, nrow=1) #### to be generalized
  colnames(output_fc) = c("point estimate of beta_fx1","point estimate of beta_fx2","point estimate of beta_fx3",
                          "Estimated RR_fx1", "Estimated RR_fx2", "Estimated RR_fx3", 
                          "Estimated variance of beta_fx1","Estimated variance of beta_fx2","Estimated variance of beta_fx3")
...
## Example syntax: fc_df(data=mydata, X="ref",S="S",H="H",W="local",Y="Y", nref=2, nstud=5, knots=c(0.15, 0.35, 0.65, 0.85),
##                       covar=c("age","bmi"))
##                 fc_df_no_cov(data=mydata, X="ref",S="S",H="H",W="local",Y="Y", nref=2, nstud=5, knots=c(0.15, 0.35, 0.65, 0.85))
